...
simulation_dir = simulation_dir + "/" + sim
...
class MessageFactory():
    ...
    def end(actual_time):
        ...
        if(protocol_version == "1.0"):
            ...
            mex.metadata = "0"
            return(mex)
    def energyCost(device,time, protocol_version):
        global jid
        global basejid
        if(protocol_version == "1.0"):
            ...
            url = basejid.split('@')[1]
            mex = Message(to=basejid+"/actormanager")
            message="ENERGY_COST [0] " + "http://"+str(url)+"/~gcdemo/"+realpath+"/" +str(es.mydir)+"/"+ str(device.profile) +" " + str(time)
            mex.body = message
            return(mex)
        else:
            mex = Message(to=basejid+"/"+jid)
            message =  """{"message" : {"subject" : "ENERGY_COST",id: "[0]","profile" : "http://parsec2.unicampania.it/~gcdemo/"""+realpath+"/"+str(es.mydir)+"""/"""+str(device.profile)+""" "}}"""
            mex.body = message
            mex.metadata = '0'
            return(mex)
	
    def energyCostProducer(device, time, protocol_version):
        global jid
        global basejid 
        if(protocol_version == "1.0"):
            name = basejid.split('@')[0]
            url = basejid.split('@')[1]
            mex = Message(to=basejid+"/actormanager")
            message="ENERGY_COST ["+str(device.house)+"]:["+str(device.device.id)+"] " + "http://"+str(url)+"/~gcdemo/"+realpath+"/Simulations/" +str(es.mydir)+"/"+ str(device.energycost) + " " + str(time)
            mex.body = message
            return(mex)
        else:
            mex = Message(to=basejid+"/"+jid)
            message =  """{"message" : {"subject" : "ENERGY_COST",id: "[""" + str(device.house) + """]:[""" + str(device.device.id) + """]","profile" : "http://parsec2.unicampania.it/~gcdemo/"""+realpath+"/"+str(es.mydir)+"""/"""+str(device.energycost)+""" "}}"""
            mex.body = message
            mex.metadata = '0'
            return(mex)
			
    def energyMix(device, time, protocol_version):
        global jid
        global basejid
        if(protocol_version == "1.0"):
            name = basejid.split('@')[0]
            url = basejid.split('@')[1]
            mex = Message(to=basejid+"/actormanager")
            message="ENERGY_MIX " + "http://"+str(url)+"/~gcdemo/"+realpath+"/" +str(es.mydir)+"/"+ str(device.profile) + " " + str(time)
            mex.body = message
            return(mex)
        else:
            mex = Message(to=basejid+"/"+jid)
            message =  """{"message" : {"subject" : "ENERGY_MIX","profile" : "http://parsec2.unicampania.it/~gcdemo/"""+realpath+"/"+str(es.mydir)+"""/"""+str(device.profile)+""" "}}"""
            mex.body = message
            mex.metadata = '0'
            return(mex)
			
    def neighborhood(device,time, protocol_version):
        global jid
        global basejid
        if(protocol_version == "1.0"):
            name = basejid.split('@')[0]
            ...
            mex = Message(to=basejid+"/"+jid)
            message =  """{"message" : {"subject" : "CREATE_ENERGY_GROUP","powerpeak" : " """ + str(device.peakload) + """ "}}"""
            mex.body = message
            mex.metadata = '0'
            return(mex)
			
    def house(device, time, protocol_version):
        global jid
        global basejid
        if(protocol_version == "1.0"):
            name = basejid.split('@')[0]
            ...
            mex = Message(to=basejid+"/"+jid)
            message =  """{"message" : {"subject" : "CREATE_ENERGY_GROUP", "id" : " """ + str(device.id) + """ ", "powerpeak" : " """ + str(device.peakload) + """ "}}"""
            mex.body = message
            mex.metadata = '0'
            return(mex)
			
    def chargingstation(device,time, protocol_version):
        global jid
        global basejid
        if(protocol_version == "1.0"):
            name = basejid.split('@')[0]
            ...
            mex = Message(to=basejid+"/"+jid)
            message =  """{"message" : {"subject" : "CREATE_ENERGY_GROUP", "id" : " """ + str(device.id) + """ ", "powerpeak" : " """ + str(device.peakload) + """ ", "numcp" : " """ + str(device.numcp) + """ "}}"""
            mex.body = message
            mex.metadata = '0'
            return(mex)

    def chargingpoint(device,time, protocol_version):
        global jid
        global basejid
        if(protocol_version == "1.0"):
            name = basejid.split('@')[0]
            url = basejid.split('@')[1]
            mex = Message(to=basejid+"/actormanager")
            message="CREATE_ENERGY_GROUP [" + str(device.houseid) + "]:[" + str(device.id) + "]" + " CONNECTORS_TYPE " + str(device.connection_type) + " POWERPEAK " + str(device.peakload) + " " + str(time)
            mex.body = message
            return(mex)
        else:
            mex = Message(to=basejid+"/"+jid)
            message =  """{"message" : {"subject" : "CREATE_ENERGY_GROUP", "id" : "[""" + str(device.houseid) + """]:[""" + str(device.id) + """]", "connectors_type" : " """ + str(device.connection_type) + """ ", "powerpeak" : " """ + str(device.peakload) + """ "}}"""
            mex.body = message
            mex.metadata = '0'
            return(mex)
    ...
    def heatercooler(device, time, protocol_version):
        ...
        if(protocol_version == "1.0"):
            ...
            url = basejid.split('@')[1]
            mex = Message(to=basejid+"/actormanager")
            message="HC ["+str(device.house)+"]:["+str(device.device.id)+"] 0 " + "http://"+str(url)+"/~gcdemo/"+realpath+"/" +str(es.mydir)+"/"+ str(device.profile) + " " + str(time)
            mex.body = message
            return(mex)
        else:
            mex = Message(to=basejid+"/"+jid)
    ...
    def background(device, time, protocol_version):
        ...
        if(protocol_version == "1.0"):
            ...
            url = basejid.split('@')[1]
            mex = Message(to=basejid+"/actormanager")
            message="BG  ["+str(device.house)+"]:["+str(device.device.id)+"] 0 " + "http://"+str(url)+"/~gcdemo/"+realpath+"/" +str(es.mydir)+"/"+ str(device.profile) + " " + str(time)
            mex.body = message
            return(mex)
        else:
    ...
    def booking_request(device, time, protocol_version):
        ...
        if (protocol_version == "1.0"):
            mex = Message(to=basejid+"/actormanager")
            message = "EV [" + str(device.device.id) + "] "+ device.Soc_at_arrival + " " + device.planned_departure_time + " " + device.actual_arrival_time + " ["+str(device.house)+"]:["+ str(device.device.cp) + "] " +  device.v2g + " "  + device.target_soc + " " + str(time)
            mex.body = message
            ...
            message = """{"message" : {"subject" : "EV" , "id" : "[""" + str(device.device.id) + """]", "soc_at_arrival": " """ + str(device.Soc_at_arrival) + """ " , "planned_departure_time" : " """ + str(device.planned_departure_time) + """ " , "arrival_time" : " """ + str(device.actual_arrival_time) + """ " ,"charging_point" : "[""" + str(device.house) + """]:[""" + str(device.device.cp) + """]", "v2g" : " """ + str(device.v2g) + """ " , "target_soc" : " """ + str(device.target_soc) + """ " }}"""
            mex.body = message
            mex.metadata = time
            return (mex)
			
    def create_ev(device, time, protocol_version):
        global jid
        global basejid
        if (protocol_version == "1.0"):
            mex = Message(to=basejid+"/actormanager")
            message = "CREATE_EV [" + str(device.device.id) + "] " + device.device.capacity + " " + device.device.max_ch_pow_ac + " " + device.device.max_ch_pow_cc + " "  + device.device.max_all_en + " " + device.device.min_all_en + " " + device.device.sb_ch + " " + device.device.sb_dis + " " + device.device.ch_eff + " " + device.device.dis_eff + " " + device.v2g + " " + str(time)
            mex.body = message
    ...
    def create_Battery(device, time, protocol_version):
        global jid
        global basejid
        if (protocol_version == "1.0"):
            mex = Message(to=basejid+"/actormanager")
            message = "CREATE_BATTERY " + "["+str(device.house)+"]:[" + str(device.device.id) + "] " +  device.device.capacity + " " + device.device.max_ch_pow_ac + " " + device.device.max_ch_pow_cc + " "  + device.device.max_all_en + " " + device.device.min_all_en + " " + device.device.sb_ch + " " + device.device.ch_eff + " " + device.Soc_at_arrival + " " + device.start_time + " " + device.end_time
            mex.body = message
    ...
    def create_producer(device, time, protocol_version):
        ...
        if(protocol_version == "1.0"):
            ...
            mex = Message(to=basejid+"/"+jid)
            message =  """{"message" : {"subject" : "CREATE_PRODUCER","type" : "PV","id" : "[""" + str(device.house) + """]:[""" + str(device.device.id) + """]"}}"""
            mex.body = message
            mex.metadata = time
            return (mex)
    ...
    def create_load(device,time, protocol_version):
        ...
        if(protocol_version == "1.0"):
            ...
            url = basejid.split('@')[1]
            mex = Message(to=basejid+"/actormanager")
            message = "LOAD [" + str(device.house) + "]:[" + str(device.device.id) + "]:[1] 1 " + str(device.est) + " " + str(device.lst) + " " +  "http://"+str(url)+"/~gcdemo/"+realpath+"/" +str(es.mydir)+"/"+ str(device.profile)  + " " + str(time)
            mex.body = message
    ...
    def update_producer(device,time,  protocol_version):
        ...
        if(protocol_version == "1.0"):
            mex = Message(to=basejid+"/actormanager")
            message="PREDICTION_UPDATE ["+str(device.house)+"]:["+str(device.device.id)+"]  " +  "http://"+str(url)+"/~gcdemo/"+realpath+"/" +str(es.mydir)+"/"+ str(device.profile) + " " + str(time)
            mex.body = message
            return(mex)
        else:
            mex = Message(to=basejid+"/"+jid)
            message =  """{"message" : {"subject" : "PREDICTION_UPDATE","type" : "PV","id" : "[""" + str(device.house) + """]:[""" + str(device.device.id) + """]","profile" : "http://parsec2.unicampania.it/~gcdemo/"""+realpath+"/"+str(es.mydir)+"""/"""+str(device.profile)+""" "}}"""
            mex.body = message
            mex.metadata = time
            return(mex)
    ...
    def delete_load(device,time,  protocol_version):
        ...
        if(protocol_version == "1.0"):
            ...
            message="DELETE_LOAD ["+str(device.house)+"]:["+str(device.device.id)+"]:[1] " + str(device.consumption) + " " + " " + str(time)
            mex.body = message
            return(mex)
        else:
            mex = Message(to=basejid+"/"+jid)
                        ...
                        if(protocol_version == "1.0"):
                            ...
                            while(messageFromScheduler.body.split(" ")[0] != "SCHEDULED"):
                                ...
                                try:
                                    ...
                                    mydel = es.eventDelete(nextload.device, nextload.house, newTime, calculate_consum(nextload.profile))
